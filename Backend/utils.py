...
def csv_writer(name, threshold_in_time):
    dir_path=r"D:\Projects\face attendance system\Backend"
    names_list = np.load(os.path.join(dir_path,r"known_faces_database\known_face_labels.npy")).tolist()
    ...
    out_csv_name_=os.path.join(dir_path,r"database\master_csv")   # D:\Projects\face attendance system\Backend\database\master_csv
    csv_out_path = os.path.join(out_csv_name_, out_csv_name)

    if out_csv_name not in os.listdir(out_csv_name_):
...
#csv_writer('Elon Musk', threshold_in_time)
